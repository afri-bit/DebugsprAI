...
class TestJSONPlaceholderFetcher(unittest.TestCase):
    ...
    def test_fetch_posts(self, mock_get):
        mock_response = [{"postId": 1, "title": "Mock Post", "content": "some content"}]
        ...
        self.assertEqual(result[0].body, "some content")

    def test_post_data_initialization(self):
        sample_data = {"postId": 1, "title": "Test Post", "content": "some content"}
        post = PostData(sample_data)

        self.assertEqual(post.post_id, sample_data["postId"])
        self.assertEqual(post.title, sample_data["title"])
        self.assertEqual(post.body, sample_data["content"])
